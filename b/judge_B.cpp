...
void Algorithm::updateDistances() {
  if (is_first == 0) {
    ...
    if (false) fprintf(stderr,
                        "Start preprocess for graph (Dijkstra method) ");
    int indicator = 1;
...
class judge {
  ...
  void out_vislog_for_turn(const Main &main, std::ostream &dest, std::vector<Command2> &command);
...
void judge::getGraphData(FILE *) {
  ...
  "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 1001, N, M);
  v.resize(N);
  v_p.resize(N);
  for (size_t i = 0; i < N; i++) {
    readLineSkipComment();
...
std::string judge::set_Algorithm_Initial_Input1(const bool submitFlag,
  ...
  if (false) fprintf(stderr, "###Debug %s(%d): before fprintf: N_sol=%zu\n",
                      "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 1807, N_sol);
  main.A_Data.numSol = N_sol;
  if (false) fprintf(stderr, "###Debug %s(%d): before fprintf: N=%zu,M=%zu\n",
                      "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 1811, N, M);
  ...
  {
    main.A_Data.edges = std::vector<std::vector<int>>(main.A_Data.numVertexes);
  ...
  if (false) fprintf(stderr,
                      ...
                      "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 2102, N_nano, Grid_Info.N_grid);
  if (false) {
...
void judge::out_vislog_for_turn(const Main &main, std::ostream &dest, std::vector<Command2> &command) {
  // nano grid data
  ...
  dest << std::flush;
  // EV data
  // charge v1 v2 d1 d2 order# o1 o2 ... 
  std::string buf;
  for (size_t i = 0; i < main.A_Data.vehicles.size(); ++i) {
    auto &v = main.A_Data.vehicles[i];
    dest << v->charge << " "
...
void judge::out_vislog_for_order(Main &main, std::ostream &dest) {
  ...
  dest << activeOrders.size() << "\n";
  for (OrderB *o : activeOrders) {
    int dist = main.dist(o->orig, o->dest);
    dest << o->orig + 1 << " " << o->dest + 1 << " " << dist << " "
      << o->startTime << " " << o->endTime << " " << o->vehicle_id + 1 << "\n";
  }
}
...
std::string judge::run_Algorithm(const bool submitFlag,
  ...
  for (size_t i = 0; i < N_vehicle; i++) {
    if (false) fprintf(stderr, "###Debug %s(%d),i=%zu\n", "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 2813,
                        i);
    int u, v;
  ...
  ;
  {
    ...
    if (submitFlag && vislog_fp) {
      std::ostringstream oss;
      out_vislog_for_turn(main, oss, commands);
...
double run_alg(const bool submitFlag,
  ...
  if (submitFlag && Common.vislog_fp) {
    ...
    fprintf(Common.vislog_fp, "%.0f %.0f %.0f %.0f %.0f %.0f\n", 
      C_balance, C_EV_tmax, C_EV_0, C_EV_return, C_grid_tmax, C_grid_0);
    // score score_trans score_ele score_env score_work
    fprintf(Common.vislog_fp, "%d %.4f %.4f %.4f %.4f\n", 
      day_score, trans_score, ele_score, env_score, work_score);
  }
  return day_score;
}
double main2() {
  ...
  for (;;) {
    ...
    Common.readLineSkipComment();
    cmd = Common.StrLine;
    if (false) fprintf(stderr, "###Debug %s(%d),cmd=%s\n", "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 3738,
                        cmd.c_str());
    int argc = StringHelper::SplitToken(cmd, " ", cmd_args);
    if (false) fprintf(stderr, "###Debug %s(%d),argc=%d\n", "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 3741,
                        argc);
    ...
    if (cmd_args[0] == "test") {
      ...
      if (false) fprintf(stderr, "###Debug %s(%d),Common.N_day=%zu\n",
                          "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 3790, Common.N_day);
      ...
      "./tmp.upload.101514.b7xpTv/judge_B/judge_B.cpp", 3838, cmd_args[0].c_str());
      exit(1);
    }
    ;
    if (submitFlg) {
